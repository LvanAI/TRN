...
from src.videotransforms import RandomResizedCropVideo
...
class TSNDataSet:
    ...
    def _load_image(self, name_video, frame_idx):
        ...
        img_path = self.root_path + "/" + name_video +"_"+ self.image_tmpl.format(frame_idx)
...
def get_categories_list(root_dir, categories_list_name):
    ...
    categories_path = Path(os.path.join(sys.path[0], categories_list_name))
...
def get_jester_dataset_meta(root_dir, images_dir_name, files_list_name):
    ...
    print("video file ", videos_dir)
    list_path = os.path.join(sys.path[0], files_list_name) 

    categories = get_categories_list(root_dir, 'class_file/category.txt')
...
def get_dataset_for_evaluation(
    ...
    transforms = GroupCompose([
        GroupOverSample(image_size, 250),
        Stack(roll=True),
        HWC2CHW(),
